...
#
        for index in tf.range(s[0]):
            ...
            a1 = tf.compat.v1.py_function(func=hankel1, inp=[0, tf.math.scalar_mul(self.wave_number, dipole_distances)],
                                          Tout=tf.complex64)

            xx = tf.cast(self.impedance, tf.complex64) * unrolled_scatterer[value[0]]
            yy = tf.cast(self.wave_number, tf.complex64) * (unrolled_scatterer[value[0]] - 1)
            b1 = xx / yy
            b1 = 1j * b1
